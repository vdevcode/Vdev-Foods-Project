      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-12">
        <div className="md:w-1/2">
          <div className="text-left md:w-4/5">
            <p className="text-green uppercase tracking-wide md:text-xl text-[.8rem] font-bold">Câu chuyện & Dịch vụ của chúng tôi</p>
            <h2 className="md:text-xl text-[.8rem] lg:text-l mt-2  md:leading-snug leading-snug ">Xây dựng lên thương hiệu Vdev Foods</h2>
            <p className="my-5 text-secondary leading-[30px] md:text-xl text-[.8rem]">
            ...
            <div className="grid sm:grid-cols-2 grid-cols-1 gap-8 items-center">
                {
                    serviceLists.map((service) => (
                        <div key={service.id} className="bg-green rounded-sm py-5 px-4 text-center space-y-2 text-white cursor-pointer">
                            {/* <img src={service.img} alt="" className=" mx-auto"/> */}
                            <h5 className="pt-3 font-semibold md:text-[1rem] text-[.8rem]"> {service.title}</h5>
                            <p className=" md:text-[1rem] text-[.8rem]">{service.des}</p>
                        </div>
                    ))
                }
            </div>
